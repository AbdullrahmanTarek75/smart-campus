...
class CampusMapController extends ChangeNotifier {
  ...
  // Map settings
  LatLng _campusCenter = LatLng(30.099738548474637, 31.248532736735118); // User-specified coordinates
  ...
  Future<void> _initializeCampusBuildings() async {
    try {
      ...
      debugPrint('Initialized ${_campusBuildings?.buildings.length} campus buildings');
    } catch (e) {
  ...
  void _loadHardcodedBuildingData() {
    try {
      ...
      final creditBuildingPolygon = creditBuildingCoords.map((coord) {
      ...
      final tBuildingsPolygon = tBuildingsCoords.map((coord) {
        return LatLng(coord[1], coord[0]); // Convert [lon, lat] to LatLng(lat, lon)
      }).toList();
      ...
      final gardenPolygon = gardenCoords.map((coord) {
        return LatLng(coord[1], coord[0]); // Convert [lon, lat] to LatLng(lat, lon)
      }).toList();
      ...
      final mainBuildingPolygon = mainBuildingCoords.map((coord) {
        return LatLng(coord[1], coord[0]); // Convert [lon, lat] to LatLng(lat, lon)
      }).toList();
      ...
      debugPrint('Error loading buildings from hardcoded data: ${e.toString()}');
      rethrow;
    }
  }
  ...
  List<List<double>> _getPolygonCoordinates(XmlElement polygonElement) {
    try {
      final outerBoundaryIs = polygonElement.findElements('outerBoundaryIs').first;
    ...
    for (var building in _campusBuildings!.buildings) {
      ...
      if (buildingHasThisFloor) {
        ...
        buildingMarkers[building.id] = Marker(
          ...
          height: building.id == "main_building" ? 40 : 25, // More height for two-line text
          point: position,
          child: building.id == "main_building" 
            ? Column(
  ...
  Future<void> _loadLocations() async {
    try {
      final String jsonData = await rootBundle.loadString('assets/data/locations.json');
      final List<dynamic> jsonList = json.decode(jsonData);
      
      _locations = jsonList.map((json) => LocationModel.fromJson(json)).toList();
  ...
  Future<void> _loadMapData() async {
    try {
      // Load the JSON data
      final String jsonString = await rootBundle.loadString('assets/data/campus_map.json');
  ...
  void _checkWithinCampus() {
    ...
    _isWithinCampus = true; // Default to true to avoid irritating users if they're close to campus
  ...
  void refreshNavigationWithFixedPoints() {
    if (_isNavigating && _destinationLocation != null && _cachedStartCoordinates != null && _cachedDestinationCoordinates != null) {
  ...
  void _updateMarkers() {
    ...
    for (final location in locationsOnFloor) {
      ...
      if (location.type == "building" && _campusBuildings != null) {
        final buildingExists = _campusBuildings!.buildings.any((b) => b.id == location.id);
        if (buildingExists) {
    ...
    if (_selectedFloor == 1) {
      final buildings = _locations.where((loc) => 
        loc.type == 'building' && !processedBuildingIds.contains(loc.id)
      ).toList();
    ...
    // Add POIs
    final pois = _locations.where((loc) => 
      loc.type == 'poi' && (loc.floor == null || loc.floor == _selectedFloor)
    ).toList();
          ...
          try {
            if (_mapData != null && _mapData!.containsKey('floors')) {
              ...
              if (floorData != null && floorData['buildings'] != null) {
                buildingHasThisFloor = (floorData['buildings'] as List).contains(building.id);
              } else {
  ...
  Future<void> downloadMapTilesForOfflineUse() async {
    ...
    try {
      ...
      // Zoom levels to download (15-19 is a reasonable range for campus navigation)
      const minZoom = 15;
      const maxZoom = 19;
      ...
      debugPrint('Will download $totalTiles tiles for offline use');
      
      // In a real implementation, you would:
      // 1. Create a database to store tiles
      // 2. Download each tile and save it
      // 3. Update progress
      ...
      debugPrint('Error downloading map tiles: ${e.toString()}');
  ...
  List<PolygonLayer> _createBuildingLayers() {
    ...
    if (floorMap.containsKey(_selectedFloor)) {
      final polygons = floorMap[_selectedFloor]!.map((building) => Polygon(
        points: building.polygon,
        color: building.color.withOpacity(0.5),
        borderColor: building.color,
  ...
  List<Polygon> _createBuildingPolygons() {
    ...
    // Remove campus boundary area to avoid white dotted lines
    /*
    if (_campusBoundary.isNotEmpty) {
      polygons.add(Polygon(
  ...
  Future<void> _loadRoomData() async {
    try {
      final String jsonData = await rootBundle.loadString('assets/data/rooms.json');
      final Map<String, dynamic> data = json.decode(jsonData);
      
      // Parse rooms
      _rooms = (data['rooms'] as List).map((roomJson) => RoomModel.fromJson(roomJson)).toList();
  ...
  void _updateRoomLabels() {
    ...
    final roomsOnCurrentFloor = _rooms.where((room) => room.floor == _selectedFloor).toList();
    
    // Debug print to check for Room 100
    for (var room in roomsOnCurrentFloor) {
      if (room.id == "room_100") {
        debugPrint('Room 100 found on floor: ${room.floor} in building: ${room.buildingId}');
        debugPrint('Room 100 coordinates: ${room.polygon}');
        debugPrint('Room 100 center: ${room.center}');
      }
    }
    ...
    for (var room in roomsOnCurrentFloor) {
      ...
      if (buildingId != 'main_building' && buildingId != 'credit_building' && buildingId != 't_buildings') {
        continue;
      }
  ...
  void startNavigationBetweenFixedPoints(dynamic startId, dynamic destinationId) {
    ...
    if (startId is String) {
      // Find by ID
      try {
        startLocation = _locations.firstWhere(
          (loc) => loc.id == startId,
        );
      } catch (e) {
