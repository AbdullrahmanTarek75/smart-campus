...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      debugPrint('📱 Setting up event controller for background checks');
      final eventController = Provider.of<EventController>(context, listen: false);
      eventController.init();
      debugPrint('📱 Event controller initialized for background checks');
    });
  }
  
  void _subscribeToNotifications() {
    // No need to subscribe to a stream anymore
    debugPrint('📱 Using simplified notification system');
    
    // No stream subscription needed as notifications are now handled directly by the system
  ...
  void _showNotification(EventModel event) {
    ...
    if (!mounted) {
      debugPrint('❌ Cannot show notification: context not mounted');
      return;
    }
    
    debugPrint('📱 Showing in-app notification: ${event.title}');
